...
export const StakeNavCard = (props: NavCard.PageNavCardProps) => {
  ...
  return (
    <NavCard.Card
      ...
      endBox={{
        title: (
          <NavCard.SecondaryText>{isLoading ? <Skeleton width={68} height={12} /> : "Available"}</NavCard.SecondaryText>
        ),
        value: <NavCard.PrimaryText>{isLoading ? <Skeleton width={82} height={16} /> : balance}</NavCard.PrimaryText>,
      }}
    />
  );
};
